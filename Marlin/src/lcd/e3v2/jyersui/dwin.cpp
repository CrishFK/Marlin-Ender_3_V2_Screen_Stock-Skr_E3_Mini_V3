...
#include "../../../inc/MarlinConfigPre.h"
  ...
  void CrealityDWINClass::Apply_shortcut(uint8_t shortcut) {
    switch (shortcut){
      // case Preheat_menu: Draw_Menu(Temperature); break;
  ...
  //
  // void CrealityDWINClass::Make_name_without_ext(char *dst, char *src, size_t max_len /*=MENU_CHAR_LIMIT*/) {
  //   size_t _pos = strlen(src);  // index of ending nul
  //
  //  // For files, remove the extension
  //  // which may be .gcode, .gco, or .g
  //  if (!card.flag.filenameIsDir)
  //    while (_pos && src[_pos] != '.') _pos--; // find last '.' (stop at 0)
  //
  //  if (!_pos) _pos = strlen(src);  // _pos = 0 ('.' not found) restore pos
  //
  //  size_t _len = _pos;   // nul or '.'
  //  if (_len > max_len) { // Keep the name short
  //    _pos = _len = max_len; // move nul down
  //    dst[--_pos] = '.'; // insert dots
  //    dst[--_pos] = '.';
  //    dst[--_pos] = '.';
  //  }
  //
    ...
    switch (menu) {
      ...
      case ManualLevel:
        ...
        case Preheat: {
          ...
          #define PREHEAT_TOTAL PREHEAT_6
          ...
          switch (item) {
            ...
            case PREHEAT_MODE:
              ...
              case PREHEAT_6:
                if (draw) {
                  Draw_Menu_Item(row, ICON_Temperature, F(PREHEAT_6_LABEL));
                }
                else 
                  do_preheat(5);
                break;
          ...
          switch (item) {
            ...
            case CHANGEFIL_PARKHEAD:
              if (draw)
                ...
                Draw_Menu_Item(row, ICON_Homing, GET_TEXT_F(MSG_FILAMENT_PARK_ENABLED));
              else {
                #if ENABLED(NOZZLE_PARK_FEATURE)
                    queue.inject(F("G28O\nG27 P2"));
                  #else
      ...
      case Control:
        ...
        switch (item) {
          ...
          case CONTROL_MOTION:
            ...
            if (draw)
              Draw_Menu_Item(row, ICON_Homing, GET_TEXT_F(MSG_FILAMENT_PARK_ENABLED), nullptr, true);
      ...
      case TempMenu:
        ...
        #define TEMP_TOTAL TEMP_PREHEAT6
        ...
        switch (item) {
          case TEMP_BACK:
            ...
            case TEMP_PREHEAT6:
              if (draw)
                Draw_Menu_Item(row, ICON_Step, F(PREHEAT_6_LABEL), nullptr, true);
              else
                Draw_Menu(Preheat6);
              break;
          #endif
        ...
        case Preheat6:
          ...
          #define PREHEAT6_BACK 0
          #define PREHEAT6_HOTEND (PREHEAT6_BACK + ENABLED(HAS_HOTEND))
          #define PREHEAT6_BED (PREHEAT6_HOTEND + ENABLED(HAS_HEATED_BED))
          #define PREHEAT6_FAN (PREHEAT6_BED + ENABLED(HAS_FAN))
          #define PREHEAT6_TOTAL PREHEAT6_FAN
          ...
          switch (item) {
            case PREHEAT6_BACK:
              if (draw)
                ...
                Draw_Menu(TempMenu, TEMP_PREHEAT6);
              break;
            #if HAS_HOTEND
              case PREHEAT6_HOTEND:
                if (draw) {
                  Draw_Menu_Item(row, ICON_SetEndTemp, GET_TEXT_F(MSG_HOTEND_TEMPERATURE));
                  Draw_Float(ui.material_preset[4].hotend_temp, row, false, 1);
                }
      ...
      case PreheatHotend:
          ...
          switch (item) {
            ...
            // #if PREHEAT_COUNT >= 6
            //   case PREHEATHOTEND_6:
            //     if (draw)
            //       Draw_Menu_Item(row, ICON_Temperature, F(PREHEAT_6_LABEL));
            //     else
            //       ui.preheat_hotend_and_fan(5);
            //     break;
            // #endif
  ...
  FSTR_P CrealityDWINClass::Get_Menu_Title(uint8_t menu) {
    switch (menu) {
      ...
      #if PREHEAT_COUNT >= 6
        case Preheat6:        
              sprintf_P(cmd, PSTR("%s %s"), PREHEAT_6_LABEL, GET_TEXT(MSG_CONFIGURATION));
              return F(cmd);
      #endif
      case Motion:
  ...
  uint8_t CrealityDWINClass::Get_Menu_Size(uint8_t menu) {
    switch (menu) {
      ...
      #endif
      #if PREHEAT_COUNT >= 6
        case Preheat6:        return PREHEAT6_TOTAL;
      #endif
